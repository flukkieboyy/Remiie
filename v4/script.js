...
function setupPagination(id, parentElement, totalPages, currentPage, onPageChange) {
    ...
    document.getElementById(`${id}-prev`)?.addEventListener('click', () => { playClick(); onPageChange(currentPage - 1); });
    document.getElementById(`${id}-next`)?.addEventListener('click', () => { playClick(); onPageChange(currentPage + 1); });
}
...
async function fetchDataFromDB() {
    try {
        ...
        if(res.ok) {
            ...
            if (dbData && dbData.schedules) schedules = dbData.schedules;
            if (dbData && dbData.galleryData) galleryData = dbData.galleryData;
        }
    } catch(e) {
...
async function saveToDB() {
    try {
        const res = await fetch('/api/data', {
            ...
            body: JSON.stringify({ token: secretAdminToken, payload: { schedules, galleryData } })
        });
        if(!res.ok) {
            alert("❌ ไม่สามารถบันทึกได้ กรุณาตรวจสอบสิทธิ์และ Vercel Database");
        }
    } catch (e) {
        alert("❌ ไม่สามารถเชื่อมต่อกับ Database (ข้อมูลแสดงแค่บนหน้าจอ)");
    }
}
...
function renderGallery() {
    ...
    document.querySelectorAll('.gallery-thumb').forEach(thumb => {
        thumb.addEventListener('click', () => {
            ...
            if(setObj) {
                currentSet = setObj.images; currentImgIdx = 0; updateViewer(); winViewer.classList.remove('d-none'); playClick();
            }
        });
    });

    setupPagination('gallery-pagination-wrapper', container, totalPages, currentGalPage, (newPage) => {
        currentGalPage = newPage; renderGallery();
    });
}
...
document.getElementById('btn-prev').addEventListener('click', () => { currentImgIdx = (currentImgIdx - 1 + currentSet.length) % currentSet.length; updateViewer(); playClick(); });
...
function renderSchedule() {
    ...
    setupPagination('schedule-pagination-wrapper', container, totalPages, currentSchedulePage, (newPage) => {
        currentSchedulePage = newPage; renderSchedule();
    });
}

// ==========================================
// 5. MOVING POINTS BACKGROUND & UI FX
// ==========================================
...
class Particle {
    constructor() {
        ...
        this.size = Math.random() * 2 + 1; this.speedX = Math.random() * 0.5 + 0.1; this.speedY = -(Math.random() * 0.5 + 0.1);
        this.color = getComputedStyle(document.documentElement).getPropertyValue('--blue-primary').trim();
    }
    update() { this.x += this.speedX; this.y += this.speedY; if (this.x > canvas.width) this.x = 0; if (this.y < 0) this.y = canvas.height; }
    draw() { ctx.fillStyle = this.color; ctx.beginPath(); ctx.arc(this.x, this.y, this.size, 0, Math.PI * 2); ctx.fill(); }
}
function initParticles() { particles = []; const numberOfParticles = (canvas.width * canvas.height) / 8000; for (let i = 0; i < numberOfParticles; i++) particles.push(new Particle()); }
function animateParticles() {
    if (!document.getElementById('intro-overlay').classList.contains('d-none')) { requestAnimationFrame(animateParticles); return; }
    ctx.clearRect(0, 0, canvas.width, canvas.height); particles.forEach(p => { p.update(); p.draw(); }); requestAnimationFrame(animateParticles);
}
...
function makeDraggable(win) {
    ...
    header.addEventListener('mousedown', (e) => {
        ...
        win.style.zIndex = ++zIndexCounter; document.body.style.userSelect = "none";
    });
    document.addEventListener('mousemove', (e) => {
        if (!isDragging) return; e.preventDefault();
        let newX = initialX + (e.clientX - startX); let newY = initialY + (e.clientY - startY);
        newX = Math.max(0, Math.min(newX, window.innerWidth - win.offsetWidth)); newY = Math.max(0, Math.min(newY, window.innerHeight - win.offsetHeight - 55));
...
document.querySelectorAll('.close-win').forEach(btn => {
    btn.addEventListener('click', (e) => {
        e.stopPropagation();
        const win = btn.closest('.win'); win.classList.add('d-none');
        document.querySelector(`.t-icon[data-id="${win.id}"]`)?.classList.remove('active');
        if(win.id === 'win-admin-login') document.getElementById('admin-pass').value = '';
    });
});
...
// ⭐ ตรวจสอบและล็อกอินผ่าน Vercel API เท่านั้น!
document.getElementById('btn-login-submit').addEventListener('click', async () => {
    const pass = document.getElementById('admin-pass').value;
    const btn = document.getElementById('btn-login-submit');
    
    if(!pass) { alert("กรุณาใส่ Password"); return; }
    ...
    try {
        const res = await fetch('/api/data', { 
            method: 'POST', 
            headers: { 'Content-Type': 'application/json' }, 
            body: JSON.stringify({ token: pass, action: "login" }) 
        });
        
        if (res.ok) {
            // ถ้ารหัสถูก (เช็คจาก API ฝั่ง Server แล้วว่าตรงกับ ADMIN_PASS)
    ...
    schList.innerHTML = schedules.map((s, idx) => `
        <div class="admin-list-item">
            <div style="line-height:1.2;">
                <strong>${s.date}</strong> <span class="text-blue">${s.title}</span><br>
                <small class="text-dim">${s.isSpecial ? 'Special Event' : s.location}</small>
            </div>
            <button class="btn-delete" onclick="deleteSchedule(${idx})"><i class="fa-solid fa-trash"></i></button>
        </div>
    `).join('');

    const galList = document.getElementById('admin-gal-list');
    galList.innerHTML = galleryData.map((g, idx) => `
        <div class="admin-list-item d-block">
            <div class="d-flex justify-content-between mb-2">
                <strong><i class="fa-solid fa-folder text-warning"></i> ${g.id}</strong>
                <button class="btn-delete" onclick="deleteGallerySet(${idx})"><i class="fa-solid fa-trash"></i> ลบทั้งเซ็ต</button>
            </div>
            <div class="d-flex gap-2" style="overflow-x:auto;">
                ${g.images.map((img, imgIdx) => `
                    <div style="position:relative; width:50px; height:50px; flex-shrink:0;">
                        <img src="${img}" style="width:100%; height:100%; object-fit:cover; border-radius:5px;">
                        <span onclick="deleteGalleryImage(${idx}, ${imgIdx})" style="position:absolute; top:-5px; right:-5px; background:red; color:white; border-radius:50%; width:15px; height:15px; display:flex; align-items:center; justify-content:center; font-size:10px; cursor:pointer;">x</span>
                    </div>
                `).join('')}
            </div>
        </div>
...
document.getElementById('btn-submit-sch').addEventListener('click', () => {
    ...
    const newSch = {
        date: document.getElementById('add-sch-date').value,
        title: document.getElementById('add-sch-title').value,
        location: document.getElementById('add-sch-loc').value,
        mapUrl: document.getElementById('add-sch-map').value,
        booth: document.getElementById('add-sch-booth').value,
        stage: document.getElementById('add-sch-stage').value,
        note: document.getElementById('add-sch-note').value,
        isSpecial: isSpecial
    };
    if(!newSch.date || !newSch.title) { alert("กรุณาใส่วันที่และชื่องาน"); return; }
    
    schedules.unshift(newSch); 
    document.querySelectorAll('#admin-sch input[type="text"]').forEach(input => input.value = '');
    document.getElementById('add-sch-special').checked = false;
    
    renderSchedule(); renderAdminLists(); saveToDB();
});

// เพิ่ม Gallery ลง Database
document.getElementById('btn-submit-gal').addEventListener('click', () => {
    ...
    if(!setId || !imgUrl) { alert("กรุณาใส่ชื่อ Set และ URL รูปภาพ"); return; }
    
    let existingSet = galleryData.find(s => s.id === setId);
    if(existingSet) existingSet.images.push(imgUrl);
    else galleryData.unshift({ id: setId, images: [imgUrl] });
    
    document.getElementById('add-gal-url').value = ''; 
    renderGallery(); renderAdminLists(); saveToDB();
});

// ==========================================
// 8. Init Boot (แก้บั๊กค้าง)
// ==========================================
window.addEventListener('load', () => {
    ...
    // โชว์หน้า Click ทันที โดยไม่ต้องรอฟังก์ชันอื่น
    if (spinner && introText && intro) {
        ...
        intro.addEventListener('click', () => {
            ...
            setTimeout(() => {
                ...
                setTimeout(() => { 
                    ...
                    if (typeof runTyping === 'function') runTyping(); 
                }, 500);
            }, 1200);
        }, { once: true });
    }
